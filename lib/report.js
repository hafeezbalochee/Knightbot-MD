...
async function report(sock, message, args) {
  ...
  if (!FOLDER_ID || !GOOGLE_KEY) {
    return sock.sendMessage(chatId, { text: '⚠️ Drive API keys are missing in Environment Variables.' });
  }
  ...
  try {
    ...
    await sock.sendMessage(chatId, { text: `🔍 Searching Drive for: ${date}.png...` }, { quoted: message });
    ...
    if (res.data.files.length > 0) {
      ...
      await sock.sendMessage(
        chatId, 
        { image: buffer, caption: `✅ *GENERATED BY HAFEEZ*\n📅 Date: ${date}` }, 
        { quoted: message }
      );

    } else {
      await sock.sendMessage(chatId, { text: `❌ Report not found for *${date}*.\REPORT \`${date} \`NOT UPLOADED.` });
    }
  } catch (error) {
    console.error("Drive Error:", error);
    await sock.sendMessage(chatId, { text: '⚠️ *Error:* ' + error.message });
  }
}
