...
const adsbCommand = require('./commands/adsb');
const adsbKarachi = require('./commands/adsb-karachi');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        switch (true) {
            case userMessage === 'ghost on': {
    if (!senderIsOwnerOrSudo) {
        await sock.sendMessage(chatId, { text: '❌ Owner only command.' }, { quoted: message });
        break;
    }
    global.ghostMode = true;
    await sock.sendMessage(chatId, { text: '👻 Ghost mode ENABLED' }, { quoted: message });
    break;
}

case userMessage === 'ghost off': {
    if (!senderIsOwnerOrSudo) {
        await sock.sendMessage(chatId, { text: '❌ Owner only command.' }, { quoted: message });
        break;
    }
    global.ghostMode = false;
    await sock.sendMessage(chatId, { text: '👻 Ghost mode DISABLED' }, { quoted: message });
    break;
}
...
case userMessage.startsWith('bh'): {
  ...
  break;
                ...
                case userMessage.startsWith('adsb'): {
                    ...
                    await adsbCommand(sock, chatId, message, args);
                    break;
                }

                case userMessage === 'adsb karachi':
                case userMessage === 'karachi': {
                    await adsbKarachi(sock, chatId, message);
                    break;
                }
